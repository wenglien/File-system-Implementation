...
int b_write (b_io_fd fd, char * buffer, int count)
    ...
    if (g_fcbArray[fd].startBlock == 0) {
        ...
        if (g_fcbArray[fd].startBlock == 0) { printf("Failed to allocate header block\n"); return -1; }
        FileHeader newHeader; memset(&newHeader, 0, sizeof(newHeader));
        newHeader.magic = FILEHEADER_MAGIC; newHeader.fileSize = 0; newHeader.dataBlockCount = 0;
        if (LBAwrite(&newHeader, 1, g_fcbArray[fd].startBlock) != 1) return -1;
    }
    FileHeader header;
    if (LBAread(&header, 1, g_fcbArray[fd].startBlock) != 1 || header.magic != FILEHEADER_MAGIC) { printf("Invalid file header\n"); return -1; }
    ...
    while (remaining > 0) {
        ...
        if (blockIndex >= header.dataBlockCount) {
            if (header.dataBlockCount >= MAX_FILE_BLOCKS) { printf("File too large\n"); break; }
            uint64_t nb = fs_allocateBlock(); if (nb == 0) break;
            header.dataBlocks[header.dataBlockCount++] = (uint32_t)nb;
        }
        uint32_t dataLBA = header.dataBlocks[blockIndex];
        char blk[BLOCK_SIZE];
        if (LBAread(blk, 1, dataLBA) != 1) memset(blk, 0, sizeof(blk));
        uint64_t can = BLOCK_SIZE - within; if (can > remaining) can = remaining;
        memcpy(blk + within, src, (size_t)can);
        if (LBAwrite(blk, 1, dataLBA) != 1) { printf("Failed to write data block\n"); break; }
        g_fcbArray[fd].currentPos += (off_t)can;
        if (g_fcbArray[fd].currentPos > (off_t)header.fileSize) header.fileSize = g_fcbArray[fd].currentPos;
        src += can; remaining -= can;
    }
    // persist header and update fcb
    if (LBAwrite(&header, 1, g_fcbArray[fd].startBlock) != 1) return -1;
...
int b_read (b_io_fd fd, char * buffer, int count)
    ...
    FileHeader header; if (LBAread(&header, 1, g_fcbArray[fd].startBlock) != 1 || header.magic != FILEHEADER_MAGIC) return -1;
    int totalRead = 0; int remaining = bytesToRead; char* dst = buffer;
    while (remaining > 0 && g_fcbArray[fd].currentPos < (off_t)header.fileSize) {
        ...
        if (blockIndex >= header.dataBlockCount) break;
        uint32_t dataLBA = header.dataBlocks[blockIndex];
        char blk[BLOCK_SIZE]; if (LBAread(blk, 1, dataLBA) != 1) return -1;
        uint64_t can = BLOCK_SIZE - within;
        uint64_t leftInFile = header.fileSize - (uint64_t)g_fcbArray[fd].currentPos;
        if (can > (uint64_t)remaining) can = (uint64_t)remaining;
        if (can > leftInFile) can = leftInFile;
        memcpy(dst, blk + within, (size_t)can);
        g_fcbArray[fd].currentPos += (off_t)can;
        dst += can; remaining -= (int)can; totalRead += (int)can;
    }
    return totalRead;
	}
...
int b_close (b_io_fd fd)
    ...
    if (fs_resolvePath(g_fcbArray[fd].filename, &dirBlock, name, sizeof(name)) == 0) {
        ...
        while (curBlock) {
            if (LBAread(&cur, 1, curBlock) != 1) break;
            ...
            if (curBlock) curBlock = cur.nextDirBlock;
        }
    }
